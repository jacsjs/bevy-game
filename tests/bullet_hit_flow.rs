...
fn bullet_hit_despawns_entities() {
    ...
    app.add_systems(
        PostUpdate,
        bevy_game::plugins::projectiles::bullets::systems::process_bullet_hits,
    );
    ...
    let bullet = app
        .world_mut()
        .spawn(bevy_game::plugins::projectiles::bullets::Bullet { damage: 1 })
        .id();
    let enemy = app
        .world_mut()
        .spawn(bevy_game::plugins::enemies::Enemy)
        .id();
